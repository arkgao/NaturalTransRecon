...
class TransRender():
    ...
    def refract_dir(self,rays_dir,normal,relative_IOR):
        ...
        Args:
            ...
            relative_IOR (torch.Tensor):            relative index of refraction (intIOR / extIOR)
        ...
        Returns:
            ...
            cos_theta_i: [batch,1]                  the cosine value of the angle between the incomling light and the normal
        """ 
    ...
    def second_bounce(self,rays_o, rays_d):
        """ 
            ...
            mask is determined by:   
                1. whether the insection is above the surface (z=0)
        ...
        # mask
        # Despite the object is constrained by mask, sometime it would grow beyond the plane, or its bottom surface will be closely attached to the plane. We drop these rays.
        end_point = rays_o + tracing_output['depth'] * rays_d
        unibody_mask = end_point[:,2] > 0
    ...
    def third_rendering(self, rays_o, rays_d):
        ...
        valid_mask = render_output['weight_sum'].squeeze(1) > 0.5
        mask = twice_mask & valid_mask
    ...
    def render_transparent(self, rays_o, rays_d):
        ...
        """
        Despite here we can render the reflection, the environment recovered in stage1 is incomplete. And the reflection on a transparent surface is weak in most cases. So we don't render the reflection.
        Strictly speaking, the precise rendering process need gamma correction, i.e., degamma the recovered environment map and gamma the rendered color. But considering the refraction coefficient is close to 1 and we ignore the reflection, we also ignore the gamma correction.
        According to our experiments, add the gamma correction and reflection can slightly improve the reconstruction quality. Add it if you really need it.
        """

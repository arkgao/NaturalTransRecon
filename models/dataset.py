...
class Dataset:
    def __init__(self, conf):
        ...
        # for convinience, we assume the plane is z=0, and the object is at z>0
        self.bounding_box = torch.tensor([[-1.0, -1.0, 0.02],[1.0, 1.0, 1]], device=self.device)
    ...
    def load_margin(self, margin_dir):
        ...
        self.margins = torch.from_numpy(self.margin_np.astype(np.float32)).cpu()  # [n_images, H, W, 1]
        ...
        for idx in range(len(self.margin_lis)):
            ...
            usample, vsample = torch.meshgrid(torch.linspace(0,self.W-1,self.W),torch.linspace(0,self.H-1,self.H),indexing='xy')
            ...
            margin_pool.append(torch.cat([rays_o.cpu(), rays_v.cpu(), mask[:,None]], dim=-1))
    ...
    def load_pred_bounding_box(self,bb_box_path):
        ...
        self.bounding_box = torch.from_numpy(bb_box).to(self.device)
        object_bbox_min = np.array([-1.01, -1.01, -1.01, 1.0])
        object_bbox_max = np.array([ 1.01,  1.01,  1.01, 1.0])
        # Object scale mat: region of interest to **extract mesh**
        object_scale_mat = np.load(os.path.join(self.data_dir, self.object_cameras_name))['scale_mat_0']
    ...
    def gen_random_inside_rays_at(self, img_idx, batch_size):
        """
        ...
        pix_idx = torch.randint(low=0, high=usample.shape[0], size=[batch_size])
        ...
        return torch.cat([rays_o.cpu(), rays_v.cpu(), color], dim=-1).cuda(),torch.stack([pixels_y,pixels_x],dim=-1).cuda()
    
    def gen_random_margin(self, batch_size):
        """
            pick random rays inside the margin
        """
        pix_idx = torch.randint(low=0, high=self.margin_pool.shape[0], size=[batch_size])
        data = self.margin_pool[pix_idx.cpu()]
        return data.cuda()
    ...
    def gen_random_rays_at(self, img_idx, batch_size, with_mask = False):
        ...
        rays_o = self.pose_all[img_idx, None, :3, 3].expand(rays_v.shape) # batch_size, 3
        if not with_mask:
            return torch.cat([rays_o.cpu(), rays_v.cpu(), color], dim=-1).cuda()    # batch_size, 9
        else:
            mask = self.masks[img_idx][(pixels_y.cpu(), pixels_x.cpu())].float()
